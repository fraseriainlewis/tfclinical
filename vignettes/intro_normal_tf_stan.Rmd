...
This example uses illustrative data from the built-in R datasets library.
...
```{r datagen,include=TRUE,echo=TRUE, eval=FALSE}
set.seed(1001)
...
```{r out_tableoutput,include=TRUE,echo=FALSE,eval=TRUE}
saved_table1 <- readRDS("precomputed/vg3_table1.rds")
saved_table1
```
...
\mu_i &= \alpha + \beta_{\text{1}} X_{i} + \beta_{\text{2}} T_{i} \\
...
```{r rstanarm,include=TRUE,echo=TRUE,eval=FALSE}
# Note on Rescaling
...
## RStan
We now reproduce the same analysis in RStan rather than rstanarm as this requires more model specific details which can then be implemented in TFP.
   - we pass the computed priors from rstanarm above which we extracted from the stanfit results object  
...
```{r vg3_plot1, include=TRUE,echo=FALSE, fig.align="center",eval=TRUE,out.width="80%"}
...
```{r vg3_text1, include=TRUE,echo=FALSE, fig.align="center",eval=TRUE,out.width="80%"}
knitr::include_graphics("precomputed/vg3_text1.png")
```
...
```{r vg3_text2, include=TRUE,echo=FALSE, fig.align="center",eval=TRUE,out.width="80%"}
knitr::include_graphics("precomputed/vg3_text2.png")
```

```{python log_prob2,include=TRUE,eval=FALSE}
...
```{r vg3_text3, include=TRUE,echo=FALSE, fig.align="center",eval=TRUE,out.width="80%"}
knitr::include_graphics("precomputed/vg3_text3.png")
```


```{python setuprunmcmc,include=TRUE,eval=FALSE}
...
```{python reshape,include=TRUE,eval=FALSE}
...
```{r vg3_plot2, include=TRUE,echo=FALSE, fig.align="center",eval=TRUE,out.width="80%"}
