...
saved_table1 <- readRDS("precomputed/vg2_table1.rds")
saved_table1
```

## RStan
The RStan workflow is somewhat more compact than the TFP equivalent, where TFP requires more lower level details to be specified whereas these are dealt with behind the scences in RStan.
### Model definition
This is somewhat simplified for ease of use, e.g. hard coded hyperparameters.  

```{r stanmodeldef,include=TRUE,echo=TRUE,eval=FALSE}
### Stan model
  BHM_stan_1 <- "data {
...
```{r stansample,echo=TRUE,results='hide',eval=FALSE}
...
### Summarize results
```{r stanresults,fig.width=10, fig.height=6, out.width="100%",eval=FALSE}
library(bayesplot)
...
```{r out_image1, include=TRUE,echo=FALSE,out.width="50%", fig.align="center",eval=TRUE}
knitr::include_graphics("precomputed/vg2_traceplot1_stan.png")
```
...
```{python initsandsteps,eval=FALSE}
...
knitr::include_graphics("precomputed/vg2_traces.png")
```
...
```{r out_vg2_mu0, include=TRUE,echo=FALSE, fig.align="center",out.width="80%",eval=TRUE}
knitr::include_graphics("precomputed/vg2_TFP_stan_comp.png")
```
