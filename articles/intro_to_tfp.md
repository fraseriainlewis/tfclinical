# Bayesian model building walkthrough 1
...
### 1.1 Example Model - Formulation

The model implemented here into TPF code is the following
$\text{logit}(\theta)$ - TBC. Also include non-centered parmeterization.

## 2. Make data available to TFP models
...
## 3. Setup python and get data from R
...
### 4.2 Define the full model

This is straightforward provided it’s done in sequence. Care is needed
in understanding whether parameters are scalars or vectors, especially
in more complex models which feature a design matrix and matrix algebra.
Simulating a realization from the model is a simple way to see what the
structure of the *state* variable of the model is. In more complex
models simulating multiple values is useful to check the structure is as
expected.
...
![](precomputed/vg1_text1.png)

### 4.3 Define the log_prog_fn
